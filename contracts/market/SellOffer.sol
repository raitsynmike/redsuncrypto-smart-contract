...
import "../token/RedSunCryptoToken.sol";

contract SellOffer is Object {

    Dispatcher dispatcher;
    RedSunCryptoToken token;
    ...
    /// @notice Before deploy this contract you must deploy dispatcher contract and set RedSunCryptoToken address to it first
    function SellOffer(address dispatcherAddress) {
        hammer = msg.sender;
        dispatcher = Dispatcher(dispatcherAddress);
        token = RedSunCryptoToken(dispatcher.currentRedSunCryptoTokenAddress());
    }
